...
void cadastroFuncionario(Funcionario *funcionarios){
	...
	bool soLetra, cargoValido, cpfIgual, cpfValido;
...
void listarFuncionariosCargo(Funcionario *funcionarios){
	...
	if(!cargoD){
		printf("ERRO! Não há funcionário registrado neste cargo no momento!\n");
	}
...
void demissao(Funcionario *funcionarios){
	...
	for (int i = 0; i < codigo; i++){
		if (funcionarios->id == funcionarios[i].posi_id){
			for (int j = funcionarios->id; j < codigo; j++){
				funcionarios[j].id = funcionarios[j + 1].id - 1;
				strcpy(funcionarios[j].nome, funcionarios[j + 1].nome);
				strcpy(funcionarios[j].cargo, funcionarios[j + 1].cargo);
				strcpy(funcionarios[j].cpf, funcionarios[j + 1].cpf);
				funcionarios[j].diasTrabalhados = funcionarios[j + 1].diasTrabalhados;
				funcionarios[j].salarioBruto = funcionarios[j + 1].salarioBruto;
				funcionarios[j].salarioLiquido = funcionarios[j + 1].salarioLiquido;
				funcionarios[j].impostoRenda = funcionarios[j + 1].impostoRenda;
				valido = true;
			}
